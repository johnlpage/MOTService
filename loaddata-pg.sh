...
PGPASSWORD=2efdaf4b59
PGSQLCMD="psql -U postgres -h johnpage-instance-1.c41swlgcxzrp.eu-west-1.rds.amazonaws.com "
MDB="mottest"
...
then
    ...
    cat test_result_2022/test_result*.csv | sort -n | sed 's/""/NULL/g' | grep -v '^"test_id"' > test_result_2021.csv
...
then
    ...
    cat test_item_2021/test_item*.csv | sed 's/""/NULL/g' tmp.csv | awk -F, '{print $1","$2","$4","$3","$5 }' | sort -n |  grep -v '^"test_id"' | awk -F, '{if (!($1 == tstid && $2 == rfrid && $3 == locationid) ) {print $1","$2","$4","$3","$5 ; }; tstid=$1; rfrid=$2; locationid=$3 }' >test_item_2021.csv
